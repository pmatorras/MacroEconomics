...
def plot_gdp_comparison(file_path, country_codes=['ESP', 'FRA']):
    ...
    plt.legend()
    plt.grid()
    plt.savefig(f"{FIGURE_DIR}gdp_since_2019_{country_suffix}.png", dpi=300, bbox_inches="tight")
    plt.close()
...
def plot_inflation(file_path, country_codes=['ESP', 'FRA']):
    ...
    plt.grid(axis='y')
    plt.savefig(f"{FIGURE_DIR}inflation_2019_{country_suffix}.png", dpi=300, bbox_inches="tight")
    plt.close()
...
def plot_cumulative_inflation(file_path, country_codes=['ESP', 'FRA']):
    ...
    plt.legend()
    plt.grid()
    plt.savefig(f"{FIGURE_DIR}cumulative_inflation_{country_suffix}.png", dpi=300, bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    ...
    print(f"✅ Plots saved as '{FIGURE_DIR}gdp_since_2019_{get_country_suffix(country_codes)}.png', "
          f"'{FIGURE_DIR}inflation_2019_{get_country_suffix(country_codes)}.png' and "
          f"'{FIGURE_DIR}cumulative_inflation_{get_country_suffix(country_codes)}.png'")
